    ...
    public function mostraDettagliEvento(array $dati, bool $isTerminato = false): void {
        ...
        if (!isset($dati['challenge'])) {
            $dati['challenge'] = [
                ...
                'nomeAttivita' => $dati['attivita'] ?? $dati['nomeAttivita'] ?? '',
        ...
        if ($isTerminato) {
            $smarty->display('dettaglio_challenge_terminato.tpl');
        } else {
            $smarty->display('dettagliChallenge.tpl');
        }
    }
}