...
<div class="carrello-container" style="padding: 20px; background: #f9f9f9;">
    <div class="container">
        
        <div style="background: #fff; border: 2px dashed #007bff; padding: 15px; margin-bottom: 30px; border-radius: 8px;">
            <h3 style="color: #007bff; margin-top: 0;"><i class="ti ti-bug"></i> Console di Test del Carrello</h3>
            <p style="font-size: 0.9rem; color: #555;">Questo pannello serve a verificare l'esposizione corretta dei metodi della classe <code>Carrello</code> integrata con Smarty.</p>
            
            <table style="width: 100%; border-collapse: collapse; font-family: monospace; font-size: 0.85rem;">
                <thead>
                    <tr style="background: #f0f0f0; text-align: left;">
                        <th style="padding: 5px; border: 1px solid #ddd;">Metodo Richiesto</th>
                        <th style="padding: 5px; border: 1px solid #ddd;">Tipo Atteso</th>
                        <th style="padding: 5px; border: 1px solid #ddd;">Valore Rilevato attuale</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="padding: 5px; border: 1px solid #ddd;">$carrello->getTotaleArticoli()</td>
                        <td style="padding: 5px; border: 1px solid #ddd;">int</td>
                        <td style="padding: 5px; border: 1px solid #ddd; color: green; font-weight: bold;">{if isset($carrello)}{$carrello->getTotaleArticoli()}{else}N/D{/if}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px; border: 1px solid #ddd;">$carrello->getSconto()</td>
                        <td style="padding: 5px; border: 1px solid #ddd;">float</td>
                        <td style="padding: 5px; border: 1px solid #ddd; color: green; font-weight: bold;">{if isset($carrello)}€{$carrello->getSconto()|number_format:2}{else}N/D{/if}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px; border: 1px solid #ddd;">$carrello->getSpedizione()</td>
                        <td style="padding: 5px; border: 1px solid #ddd;">float|null</td>
                        <td style="padding: 5px; border: 1px solid #ddd; color: green; font-weight: bold;">
                            {if isset($carrello)}
                                {if $carrello->getSpedizione() === null}null (Da calcolare){else}€{$carrello->getSpedizione()|number_format:2}{/if}
                            {else}N/D{/if}
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 5px; border: 1px solid #ddd;">$carrello->getTotale()</td>
                        <td style="padding: 5px; border: 1px solid #ddd;">float</td>
                        <td style="padding: 5px; border: 1px solid #ddd; color: green; font-weight: bold;">{if isset($carrello)}€{$carrello->getTotale()|number_format:2}{else}N/D{/if}</td>
                    </tr>
                    <tr>
                        <td style="padding: 5px; border: 1px solid #ddd;">$carrello->getItems()|@count</td>
                        <td style="padding: 5px; border: 1px solid #ddd;">int (Conteggio Array)</td>
                        <td style="padding: 5px; border: 1px solid #ddd; color: green; font-weight: bold;">{if isset($carrello)}{$carrello->getItems()|@count}{else}0{/if} elementi in sessione/DB</td>
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                <div class="carrello-main">
                    ...
                    <div class="carrello-items" id="carrello-items">
                        {foreach $carrello->getItems() as $item}
                            ...
                            <div class="test-item-assertions" style="background: #fff3cd; font-family: monospace; font-size: 0.75rem; padding: 4px 10px; border: 1px solid #ffeeba; border-bottom: none;">
                                [TEST ARTICOLO ID: {$item->getIdItem()}] 
                                <strong>idItem:</strong> {$item->getIdItem()} | 
                                <strong>Quantità:</strong> {$qty} | 
                                <strong>Subtotale Item:</strong> €{$item->getSubtotale()|number_format:2} | 
                                <strong>ID Prodotto Associato:</strong> {$p->getIdProdotto()}
                            </div>
                    ...
                    {if isset($correlati) && $correlati|@count > 0}
                        <section class="carrello-correlati">
                            <h2 class="carrello-section-title">Potrebbe interessarti (Test Array Correlati)</h2>
                    ...
                    <dl class="carrello-summary-list">
                        ...
                        {if $carrello->getSconto() > 0}
                            <div class="carrello-summary-row carrello-summary-sconto">
                                <dt>Sconto applicato classe Carrello</dt>
        ...
        {else}
            ...
            {* ── STATO CARRELLO VUOTO RILEVATO DALLA CLASSE ── *}
            <div class="carrello-vuoto" style="border: 2px solid #dc3545; padding: 30px; text-align: center; border-radius: 8px; background: #fff;">
                <i class="ti ti-shopping-cart-off" style="font-size: 3rem; color: #dc3545;"></i>
                <p style="font-weight: bold; margin-top: 15px;">La condizione d'istanza della classe Carrello risulta VUOTA oppure non configurata correttamente (<code>$carrello</code> non passato).</p>
                <a href="{$base_url}/catalogo" class="btn-primary" style="display: inline-block; padding: 10px 20px; background: #007bff; color: #fff; text-decoration: none; border-radius: 4px; margin-top: 10px;">Vai al Catalogo</a>
            </div>
...
{block name="extra_js"}
<script>
// Manteniamo intatta la logica JavaScript originale del tuo template per testare anche il comportamento reattivo e le rotte asincrone / Fetch del carrello
function initCarrelloPage() {
    ...
    document.querySelectorAll('.carrello-item').forEach(riga => {
        ...
        function inviaAggiornamento() {
            if (!updateUrl) return;
            const qty = parseInt(input.value) || 1;
            console.log("TEST CONSOLE: Invio Fetch verso -> " + updateUrl + '?qty=' + qty);
    ...
    document.querySelectorAll('.carrello-item-rimuovi').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (url) {
                console.log("TEST CONSOLE: Invio Fetch per rimozione -> " + url);
                fetch(url).catch(err => {
            ...
            if (riga) {
                // Rimuove visivamente sia l'oggetto che la barra di debug associata
                const debugBar = riga.previousElementSibling;
                if(debugBar && debugBar.classList.contains('test-item-assertions')) {
                    debugBar.remove();
                }
                riga.remove();
            }
