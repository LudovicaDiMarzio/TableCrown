...
class FPortaDadi{
        ...
        try{
            ...
            if (!empty($filtri['disponibilita'])) {
                ...
                if (!empty($enumDisponibilita)) {
                    $qb->andWhere($qb->expr()->in('p.disponibilitaProdotto', ':disponibilita'))
     ...
     * @throws Exception
    */
    public static function ricercaPortaDadi(string $StringaDiRicerca, int $limit, int $offset): array{
        ...
        try{
            ...
            $qb->select('p')
                ->from(EPortaDadi::class, 'p')
                ->where('p.nomeProdotto LIKE :ricerca OR p.descrizioneProdotto LIKE :ricerca')
